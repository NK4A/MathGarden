...
function predictImage() {
    ...
    //find the center of mass of the immage
    cv.findContours(image, contours, hierarchy, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE);
    cnt = contours.get(0);
    ...
    // shift the image so that the centre of mass is in the centre of the canvas
    ...
    //Create our Tensor to predict the value
    const X = tf.tensor([pixelValues]);
    // console.log(`Shape of tensor ${X.shape}`)
    // console.log(`dtype of tensor ${X.dtype}`)

    const result = model.predict(X);
    console.log(`Numarule este: ${result}`)

    const output = result.dataSync()[0];


    // //Delete this later - only to test cv
